...
class FinanlizeCheckoutView(CartOrderMixin, View):
  def post(self, request, *args, **kwargs):
    ...
    print("this is the order object", dir(order))
    # print("oba ", order._address)
    nonce = request.POST.get("payment_method_nonce")
    if nonce:
      print("order object ", order)
      print("order object ", order.order_total)
      print("order object ", order.billing_address)
...
class CheckOutApiView(FormMixin,CartOrderMixin, View):
  def post(self, request, *args, **kwargs):
    ...
    if form.is_valid():
      ...
      country = form.cleaned_data.get("phone")
      checkout_address, created = UserAddress.objects.get_or_create(
      ...
      order.billing_address = checkout_address
      order.save()
      print("is this created", created)
      print("this is the usseradress id", checkout_address.id)
      print("order", order.billing_address.zipcode)
      data = {
        "works": True,
